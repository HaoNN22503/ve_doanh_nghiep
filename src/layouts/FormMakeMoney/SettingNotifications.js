...
export default function SettingNotifications() {
    ...
    const [closednoti,setClosedNoti] = useState(true)
    ...
    function showPropUp () {
     ...
     function closedFile (id) {
        setClosedFileNoti(filterOutData.map((sd) => {
            let value = false;
            if (id === sd.id) {
                sd.onClose = false;
            }
    ...
    <div className="cotainer-notifi">
        <div className="notifi-heading">
            ...
            <div className="option-notifi">
                <input type="text" placeholder="Tìm kiếm" className="notifi-search"></input>
                <div className="notifi-change">
                    <div className="notifi-change-info">Short by:</div>
                    <div className="notifi-change-info-1">Priority</div>
                    <div className="notifi-change-info-icon"><IoMdArrowDropdown/></div>
                </div>
            </div>
        </div>
        <div className="notifi-body">
            <div className="notifi-body-tittle">
                <div className="notifi-body-tittle-1"><p>Tên khách hàng</p> <div onClick={showPropUp} className="notifi-body-tittle-icon">{ sortcustomername ? <IoMdArrowDropup/> 
                : <IoMdArrowDropdown/>}</div>
                </div>
                <div className="notifi-body-tittle-1"><p>Môn học</p> <div className="notifi-body-tittle-icon"><IoMdArrowDropdown/></div>
                </div>
                <div className="notifi-body-tittle-1"><p>Gửi tới</p> <div className="notifi-body-tittle-icon"><IoMdArrowDropdown/></div>
                </div>
                <div className="notifi-body-tittle-1"><p>Thời gian gửi</p> <div className="notifi-body-tittle-icon"><IoMdArrowDropdown/></div>
                </div>
                <div className="notifi-body-tittle-1"><p>Trạng thái</p><div className="notifi-body-tittle-icon"><IoMdArrowDropdown/></div>
                </div>
            </div>
             <div> 
                ...
                <div className="notifi-status">
                    <div onClick={() => {closedFile(element.id)}} className="notifi-status-option">{element.status}</div>
                    <div className="notifi-status-icon"><BiDotsVerticalRounded/></div>
                </div>
            </div>)})}
            </div>
        </div>
